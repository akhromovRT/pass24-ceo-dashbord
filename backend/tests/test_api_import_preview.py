...
class TestCommit:
    ...
    def test_commit_payments_source_type(self, client, db_session: Session):
        ...
        ws.append([
            "Есть файлы", "Номер", "Дата", "Поступление", "Списание",
            "Контрагент", "ИНН", "Договор", "Вх.номер", "Вх.дата",
            "Назначение платежа", "Вид операции",
        ])
        # Строка 2 — один платёж (с периодом в назначении)
        ws.append([
            "Нет", "00БП-1", "15.05.2026", 12100.0, None,
            "ООО Ромашка", "7700000001",
            "Договор №1", "5", "15.05.2026",
            "Оплата за доступ к PASS24 за май 2026", "Оплата от покупателя",
        ])
        buf = io.BytesIO()
        wb.save(buf)
