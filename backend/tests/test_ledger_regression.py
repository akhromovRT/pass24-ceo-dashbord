...
def test_advance_payment_does_not_inflate_target_month(db_session):
    org = Organization(
        inn="7700000099", name_1c="Регресс", status=OrgStatus.ACTIVE, monthly_ap=Decimal("10000")
    )
    db_session.add(org)
    db_session.flush()
    db_session.add(
    ...
    march_charge = db_session.exec(
        select(MonthlyCharge).where(MonthlyCharge.year == 2026, MonthlyCharge.month == 3)
    ).first()
    collected_march = sum(
        (
            a.allocated_amount
            for a in db_session.exec(
                select(PaymentAllocation).where(
                    PaymentAllocation.monthly_charge_id == march_charge.id
                )
            ).all()
        ),
        Decimal("0"),
