...
def populated(db_session: Session):
    ...
    o_stop = _org(db_session, "INV900", name="Stopped", status=OrgStatus.ACTIVE)
...
def _control(client, metric: str, period: str | None = None):
    ...
    return client.post("/api/v1/reports/composition/preview", json=payload).json()["control_value"]
