...
def _ledger_org(db_session: Session, inn="7710000001"):
    ...
    org = Organization(inn=inn, name_1c="Орг", status=OrgStatus.ACTIVE, monthly_ap=Decimal("10000"))
    db_session.add(org)
    db_session.flush()
    contract = Contract(organization_id=org.id, contract_number="1C-PAYMENTS", raw_name="payments")
...
def test_tariff_history_get_and_post(client, db_session: Session):
    ...
    assert resp.json() == []
    new = client.post(
        f"/api/v1/organizations/{org.inn}/tariffs",
