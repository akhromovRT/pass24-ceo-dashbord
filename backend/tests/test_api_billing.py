...
def test_segments_counts(client, db_session: Session):
    ...
    body = resp.json()
    for key in ("total", "mrr_plan", "paying", "partial", "not_paying", "debtors"):
        assert key in body
    assert body["total"] == 1
...
def test_segments_objects_total_counts_client_objects(
    ...
    org2 = Organization(
        inn="7700000012",
        name_1c="С заявленными 5",
        status=OrgStatus.ACTIVE,
        monthly_ap=Decimal("10000"),
        in_registry=True,
        objects_count_declared=5,
    )
    org3 = Organization(
        inn="7700000013",
        name_1c="Без объектов",
        status=OrgStatus.ACTIVE,
        monthly_ap=Decimal("10000"),
        in_registry=True,
    )
