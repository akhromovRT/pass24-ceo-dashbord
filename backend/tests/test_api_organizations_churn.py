...
def client(db_session: Session):
    ...
    def override_get_current_user():
        return User(
            name="Test", email="t@e", hashed_password="x",
...
def _org_with_payment(db_session: Session, last_payment_date: date | None) -> Organization:
    org = Organization(
        inn="7700001234", name_1c="Тест", status=OrgStatus.ACTIVE,
    ...
    if last_payment_date is not None:
        contract = Contract(
            organization_id=org.id, contract_type=ContractType.SUBSCRIPTION,
            raw_name="Договор",
        )
        ...
        db_session.add(Document(
            ...
            doc_type=DocType.PAYMENT, doc_date=last_payment_date,
            amount=Decimal("10000"), raw_name="Оплата",
        ))
    db_session.commit()
    return org
...
def test_active_to_churned_autosets_churn_month_from_last_payment(
    ...
    resp = client.patch(
        "/api/v1/organizations/7700001234", json={"status": "churned"},
    )
...
def test_active_to_churned_with_explicit_churn_month_uses_it(
    ...
    assert resp.json()["churn_month"] == "2025-04-01", (
...
def test_patch_unrelated_field_does_not_clear_churn_month(
    ...
    resp = client.patch(
        "/api/v1/organizations/7700001234", json={"name_display": "Новое имя"},
    )
    assert resp.status_code == 200
    body = resp.json()
