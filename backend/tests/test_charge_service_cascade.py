...
def test_rebuild_clears_charges_with_existing_allocations(db_session: Session):
    ...
    org = Organization(
        inn="7700000999", name_1c="Test", status=OrgStatus.ACTIVE, monthly_ap=Decimal("10000")
    )
    db_session.add(org)
    ...
    contract = Contract(
        organization_id=org.id, contract_type=ContractType.SUBSCRIPTION, raw_name="x"
    )
    ...
    db_session.add(
        PaymentAllocation(
            ...
            monthly_charge_id=old_charge.id,
