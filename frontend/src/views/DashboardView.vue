...
const HINTS: Record<string, string> = {
  ...
  stopped: 'Клиенты, ушедшие в отток с начала года — со статусом «Отток», ' +
    'проставленным вручную после сверки транзакций в 1С. Машина не считает ' +
    'оттоком тех, кто просто задержал оплату: это решает Софья по 1С.',
  overdue: 'Активные клиенты с просрочкой оплаты: последний платёж более 60 дней ' +
    'назад и подписка не оплачена вперёд. Это сигнал для взыскания, НЕ отток — ' +
    'клиент остаётся активным. Разбирать в разделе «Должники».',
    ...
    <section class="kpi-section" v-if="summary">
      ...
      <div class="kpi-grid kpi-grid-6">
        ...
        <KpiTile
          ...
          sub="статус «Отток» (по 1С)"
          :hint="HINTS.stopped"
          accent="danger"
          :to="compositionLink('stopped_since_year_start')"
        />
        <KpiTile
          label="Активные с просрочкой"
          :value="fmt(summary.active_overdue)"
          sub="не платят >60 дней (взыскание)"
          :hint="HINTS.overdue"
          accent="warn"
          :to="compositionLink('active_overdue')"
        />
        <KpiTile
          label="Churn rate"
