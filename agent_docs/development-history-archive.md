...
**Контекст:** этап 3 пятиэтапного плана работы с импортом 1С. Дать CEO/бухгалтерии открыть конкретный импорт «как в Excel-файле 1С» и убедиться, что всё разнеслось правильно.

**Что сделано:**
- Backend `debt_snapshots.py`: `GET /debt-snapshots` (список), `/latest` (последний + сверка с БД), `/{id}`. Сверка: для каждой buyer-строки сравнивается `DebtSnapshotRow.debt_end` (1С) с `Organization.total_debt` (БД), расхождения >1 ₽ в `diffs`.
- Frontend `DebtorsView.vue`: `SelectButton` «Реестр должников ↔ 1С-вид», выбор импорта, поиск, toggle «Только расхождения», `SegmentBand` сводка, `TreeTable` Покупатель→Договор→Документ (10 колонок как в Excel), подсветка расхождений.
- Bugfix этапа 2: `_sum` возвращал `Decimal(0) or None == None`; заменён на track-seen.
- Тесты: +4 API (`test_debt_snapshots_api.py`). Suite — 245 passed.

### 2026-05-27 — Этап 2: модель DebtSnapshot и сохранение полного среза 1С при импорте

**Контекст:** этап 2 пятиэтапного плана работы с импортом 1С. Цель — на каждый импорт файла «Задолженность покупателей» сохранять полный срез: Покупатель → Договор → Документ × все 8 числовых колонок. Нужно для UI «1С-вид» (этап 3) и автосверки (этап 4).

**Что сделано:**
- Новые таблицы `debt_snapshots` и `debt_snapshot_rows` (`backend/app/models/debt_snapshot.py`): `DebtSnapshot` 1-к-1 с `ImportRun`; `DebtSnapshotRow` — строка файла с `parent_row_id`, `row_index`, 8 числовых полей и FK на Organization/Contract/Document.
- Миграция `a3f1b9c47e02_add_debt_snapshots.py` (`d8e2b91f5a7c → a3f1b9c47e02`), enum `debtsnapshotlevel`, CASCADE/SET NULL.
- Интеграция в `import_service.process_import`: `_build_debt_snapshot(...)` пишет дерево в той же транзакции; `_process_document` распилен на `_build_document` + `_add_document`.
- Backfill-скрипт `backend/scripts/backfill_debt_snapshot.py`.
- Тесты (`test_debt_snapshot.py`, 6 новых). Прогон: 241 passed, 9 skipped.
