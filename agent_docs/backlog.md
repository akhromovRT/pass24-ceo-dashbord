...
- Сейчас в `DebtorsView.vue` режим «1С-вид» есть поиск (имя/ИНН/№ док/№ договора) и
  toggle «Только расхождения». Этого хватает для базовой сверки.
- **Доп. фильтры:**
...
- Текущее: **245 passed, 9 skipped**. Skipped — file-based парсер-тесты (нет тестовых артефактов в репо).
