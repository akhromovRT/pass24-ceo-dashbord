...
- Деплой на production — требует подтверждения пользователя; ручная браузерная
  проверка ожидает запуска стека.

**Следующий шаг:** деплой модуля и ручная проверка; решение по сетевому дефекту VPS.
