...
- Задолженность покупателей (свежий пример): `~/Downloads/Задолженность покупателей за 01.01.2025 - 24.05.2026 ООО  ОНВИ СЕРВИС.xls` (.xls формат — парсер поддерживает через xlrd; парсер починен 2026-05-26, см. ADR-019). После импорта в системе — UI «1С-вид» в разделе `/debtors?mode=source`.
...
- `agent_docs/guides/registry-reconciliation-mpp.md` — инструкция для МПП (Софья): ежемесячная сверка Excel-реестра клиентов с системой CEO24, разбор расхождений (статус, АП, транзиты, дубли).
